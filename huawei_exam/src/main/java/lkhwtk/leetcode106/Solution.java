...
/**
 * 106. 从中序与后序遍历序列构造二叉树
 * 本题参考了105题思路，主要区别就一个：先序遍历的根结点在前边，后序遍历的根节点在后边，注意下递归的边界
 */
public class Solution {
     ...
     * @param inorder_right 中序遍历右端结束位置，包含右端
     */
    public TreeNode myBuildTree(int[] postorder, int postorder_left, int postorder_right, int inorder_left, int inorder_right) {
        ...
        root.left = myBuildTree(postorder, postorder_left, postorder_left+size_left_subtree-1, inorder_left, inorder_root - 1);
        // 递归地构造右子树，并连接到根节点
        // 后序遍历中「从 左边界+size_left_subtree 开始到 右边界-1」的元素就对应了中序遍历中「从 根节点定位+1 到 右边界」的元素
        root.right = myBuildTree(postorder, postorder_left+size_left_subtree, postorder_right-1, inorder_root + 1, inorder_right);
        return root;
    }
